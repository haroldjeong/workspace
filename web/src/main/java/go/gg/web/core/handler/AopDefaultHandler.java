...
public class AopDefaultHandler {
	...
	// 예시 : go.gg.web.apps 패키지 하위에 controller 패키지에 속하고 Impl로 끝나는 모든 클래스의 모든 메소드(파라미터 갯수가 0개 이상이며 어떠한 타입의 어떠한 이름의 파라미터여도 상관없음) 실행 시 아래 로직 실행 (jm.lee)
	@Pointcut("execution(* go.gg.web.apps..controller.*Impl.*(..))")
